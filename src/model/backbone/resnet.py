...
class BottleNeck(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1):
        ...
        # 瓶颈残差块（BottleNeck，对应 ResNet-50/101/152）：
        # - 1x1 卷积将通道压缩到 out_channels（降维，降低 3x3 计算量）
        # - 3x3 深度卷积（groups=out_channels）提取空间特征（stride 控制是否下采样）
        # - 1x1 卷积保持/混合通道
        # - 最后用 1x1 卷积将通道扩展到 out_channels * expansion（expansion=4），
        #   使输出通道数为输入的 4 倍，增强表示能力。
        self.residual_function = nn.Sequential(
