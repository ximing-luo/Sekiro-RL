...
from ..components import RMSNorm, RMSNorm2d, GatedMLP, ResBlock, SEBlock
class StaticBranch(nn.Module):
    """
    静态分支 (Spatial Stream)
    """
    def __init__(self, in_channels, channel_multiplier=1.0):
        super().__init__()
        ...
        self.conv2 = nn.utils.spectral_norm(
            nn.Conv2d(2, int(8 * m), kernel_size=5, stride=2, padding=2, bias=False)
        )
        self.act = nn.SiLU(inplace=True)
        self.norm2 = RMSNorm2d(int(8 * m))
        self.se = SEBlock(int(8 * m))
        
        self.in_channels = int(8 * m)
        self.backbone = nn.Sequential(
            self._make_layer(int(8 * m), int(8 * m), 1, 1),
            self._make_layer(int(32 * m), int(32 * m), 2, 2),
            self._make_layer(int(128 * m), int(32 * m), 1, 2),
            self._make_layer(int(256 * m), int(64 * m), 1, 2),
            self._make_layer(int(512 * m), int(128 * m), 1, 2),
            self._make_layer(int(256 * m), int(128 * m), 1, 2),
        )
        
        self.norm3 = RMSNorm2d(int(256 * m))
        self.head = nn.Sequential(
            nn.utils.spectral_norm(nn.Conv2d(int(256 * m), int(64 * m), kernel_size=1, bias=True)),
    ...
    def _make_layer(self, out_channels, mid_channels, num_block, stride):
        layers = []
        layers.append(ResBlock(self.in_channels, out_channels, mid_channels, stride))
        self.in_channels = out_channels
        for _ in range(num_block - 1):
            layers.append(ResBlock(self.in_channels, out_channels, mid_channels, 1))
...
class DynamicBranch(nn.Module):
    ...
    def __init__(self, in_channels, channel_multiplier=1.0):
        ...
        self.backbone = nn.Sequential(
            self._make_layer(int(32 * m), int(32 * m), 2, 2),
            self._make_layer(int(64 * m), int(64 * m), 2, 2),
            self._make_layer(int(128 * m), int(512 * m), 4, 2),
            self._make_layer(int(64 * m), int(512 * m), 2, 2),
        )
        
        self.norm3 = RMSNorm2d(int(64 * m))
        self.head = nn.Sequential(
            nn.utils.spectral_norm(nn.Conv2d(int(64 * m), int(32 * m), kernel_size=1, bias=True)),
            nn.SiLU(inplace=True),
            nn.Flatten()
        )
...
class SekiroMADSExtractor(BaseFeaturesExtractor):
    ...
    双流 PPO 特征提取器 (MADS)
    """
    def __init__(self, observation_space, features_dim=512, channel_multiplier=1.0):
        super().__init__(observation_space, features_dim)
        m = channel_multiplier
        
        self.spatial_cnn = StaticBranch(3, m)
        self.temporal_cnn = DynamicBranch(3, m)
    ...
    def _init_weights(self, module):
        if isinstance(module, nn.Linear):
            ...
            # 兼容 spectral_norm (weight -> weight_orig)
            if hasattr(module, "weight_orig"):
                torch.nn.init.normal_(module.weight_orig, mean=0.0, std=0.02)
            else:
                torch.nn.init.normal_(module.weight, mean=0.0, std=0.02)
            if module.bias is not None: torch.nn.init.zeros_(module.bias)
